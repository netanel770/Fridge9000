...
try:
    ...
    from services import annotation_sessions, annotations, households
except ModuleNotFoundError:
    from backend.api.dependencies import get_active_household, get_system_admin
    from backend.services import annotation_sessions, annotations, households
...
def create_submission(scan_id: int, payload: dict[str, Any], context: Household):
    return annotation_sessions.create_annotation_submission(
...
def my_submissions(context: Household, status: str | None = None):
    return annotation_sessions.list_my_annotation_submissions(
...
def my_submission(submission_id: int, context: Household):
    return annotation_sessions.get_my_annotation_submission(
...
router.add_api_route(
    ...
    annotation_sessions.list_annotation_submissions,
    methods=["GET"],
    dependencies=_admin_only,
)
router.add_api_route(
    "/annotation-submissions/stats",
    annotations.get_annotation_submission_stats,
    methods=["GET"],
    dependencies=_admin_only,
)
router.add_api_route(
    "/annotation-submissions/{submission_id}",
    annotation_sessions.get_annotation_submission,
    methods=["GET"],
    dependencies=_admin_only,
)
router.add_api_route(
    "/annotation-submissions/{submission_id}/image",
    annotations.get_annotation_submission_image,
    methods=["GET"],
    dependencies=_admin_only,
)
router.add_api_route(
    "/annotation-submissions/{submission_id}",
    annotations.update_annotation_submission,
    methods=["PATCH"],
    dependencies=_admin_only,
)
router.add_api_route(
    "/annotation-submissions/{submission_id}/quarantine",
    annotations.update_quarantined_submission,
    methods=["POST"],
    dependencies=_admin_only,
)
