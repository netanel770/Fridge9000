...
import { updateInventoryByImage } from "../src/services/api";
...
export default function UpdateInventoryScreen() {
  ...
  async function uploadImage() {
    ...
    try {
      const res = await updateInventoryByImage(imageUri, mode);

      const message = res.updated_items
        ?.map((item: any) => `${item.name}: ${item.new_quantity}`)
        .join("\n");
