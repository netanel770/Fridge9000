...
export type SubmissionLabelGroup = { label: string; submissions: AnnotationSubmissionDetail[] };
...
export function readableModelName(model: ModelIdentity | null | undefined, displayNames: Record<string, string>) {
...
export function promotionReasonText(reason: PromotionReason) {
  ...
  if (reason.code === "added_class_below_minimum" && reason.classes && !Array.isArray(reason.classes) && reason.minimum != null) {
    return `${Object.keys(reason.classes).join(", ")} must reach at least ${formatMetric(reason.minimum)} mAP50–95.`;
  }
...
export function metricsForProduct(metrics: AddedClassMetrics | undefined, product: string) {
...
export function groupSubmissionsByLabel(submissions: AnnotationSubmissionDetail[]): SubmissionLabelGroup[] {
