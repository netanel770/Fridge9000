...
export function ModerationQueue({ submissions, loading, error, message, moderatingSubmissionId, expandedAnnotationIds, onReload, onModerate, onToggleDetails }: {
  ...
  return <>
    ...
    {!loading && submissions.map((detail) => <Card key={detail.submission.id}>
      ...
      <DetectionImageViewer imageUri={getAnnotationSubmissionImageUrl(detail.submission.id)} imageWidth={detail.submission.image_width} imageHeight={detail.submission.image_height} detections={detail.annotations.map((annotation) => contributionDetection({ submission: detail.submission, annotation }))} style={styles.moderationImage} />
